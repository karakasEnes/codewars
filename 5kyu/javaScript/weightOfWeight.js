...
function orderWeight(strng) {
  const state = strng.split(' ').reduce((prev, cur) => {
    const totalOfCur = cur.split('').reduce((prev, cur) => prev + +cur, 0);
    prev[totalOfCur] ? prev[totalOfCur].push(cur) : (prev[totalOfCur] = [cur]);
    return prev;
  }, {});

  return Object.entries(state)
    .reduce((prev, [k, v]) => {
      v.length > 1 ? v.sort().forEach((el) => prev.push(el)) : prev.push(v[0]);
      return prev;
    }, [])
    .join(' ');
}
